...
nurseSchema.statics.encryptPassword = async (password) => {
  const salt = await bcrypt.genSalt(10);
  return await bcrypt.hash(password, salt);
};

nurseSchema.statics.comparePassword = async (password, receivedPassword) => {
...
nurseSchema.pre('save', async function (next) {
  ...
  const hash = await bcrypt.hash(nurse.password, 10);
  nurse.password = hash;
  next();
});
